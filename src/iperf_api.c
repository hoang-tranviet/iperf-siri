...
iperf_on_test_start(struct iperf_test *test)
{
	...
	if (test->requested_subflows != NULL)
            cJSON_AddStringToObject(test->json_start, "requested_subflows", test->requested_subflows);
...
iperf_send(struct iperf_test *test, fd_set *write_setP)
{
    ...
    while (multisend > 0) {
        ...
        SLIST_FOREACH(sp, &test->streams, streams) {
            if (test->debug)
                printf ("%lu.%lu: %d %d, %d %d %d: ",now.tv_sec %10, now.tv_usec,
...
/*
 * We modify send_results() and get_results() to handle
 * statistics of subflows in place of TCP streams
 */
/*************************************************************/
...
send_results(struct iperf_test *test)
{
	...
	if (test->role == 's' && test->get_server_output) {
        ...
        /* remote iperf is stock: send TCP stream results */
        if (!test->remote_iperf_supports_mptcp) {
            cJSON *j_streams;
            struct iperf_stream *sp;
            cJSON *j_stream;
...
connect_msg(struct iperf_stream *sp)
{
    ...
    if ((sp->test->json_output) || (sp->test->remote_iperf_supports_mptcp))
...
iperf_stats_callback(struct iperf_test *test)
{
    ...
    /* get list of subflows and check if MPTCP is enabled */
    sp = SLIST_FIRST(&test->streams);
...
/**
 * Print intermediate results during a test (interval report).
 * Uses print_sf_interval_results to print the results for each subflow,
 * then prints an interval summary for all subflows in this interval.
 */
static void
iperf_print_sf_intermediate(struct iperf_test *test)
{
    char ubuf[UNIT_LEN];
    char nbuf[UNIT_LEN];
    ...
    if (json_interval_streams == NULL)
        return;
    cJSON_AddItemToObject(json_interval, "subflows", json_interval_streams);

    SLIST_FOREACH(sf, &test->subflows, subflows) {
        print_sf_interval_results(test, sf, json_interval_streams);
	irp = TAILQ_LAST(&sf->result->interval_results, irlisthead);
	if (irp == NULL) {
	    // iperf_err(test, "iperf_print_sf_intermediate error: interval_results is NULL");
	    return;
	}
        bytes += irp->bytes_transferred;
	    if (test->sender && test->sender_has_retransmits)
...
/**
 * Print overall summary statistics at the end of a test.
 * Was modified to work with MPTCP subflow.
 */
static void
iperf_print_sf_results(struct iperf_test *test)
{

    cJSON *json_summary_subflows = NULL;
    cJSON *json_summary_subflow = NULL;
    int total_retransmits = 0;
    char ubuf[UNIT_LEN];
    char nbuf[UNIT_LEN];
    //struct stat sb;
    //char sbuf[UNIT_LEN];
    struct iperf_subflow *sp = NULL;
    iperf_size_t bytes_sent, total_sent = 0;
    iperf_size_t bytes_received, total_received = 0;
    double start_time, end_time;
    double bandwidth;
    ...
    json_summary_subflows = cJSON_CreateArray();
    if (json_summary_subflows == NULL)
        return;
    cJSON_AddItemToObject(test->json_end, "subflows", json_summary_subflows);
     ...
     * we were running the tests and print out some statistics about
     * the subflows.  It's possible to not have any subflows at all
     * if the client got interrupted before it got to do anything.
     */
    if (sp) {
    end_time = timeval_diff(&sp->result->start_time, &sp->result->end_time);
    ...
    {
        json_summary_subflow = cJSON_CreateObject();
        if (json_summary_subflow == NULL)
            return;
        cJSON_AddItemToArray(json_summary_subflows, json_summary_subflow);
	...
	if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
	    if (test->sender_has_retransmits) {
            /* Summary, TCP with retransmits. */
                cJSON_AddItemToObject(json_summary_subflow, "sender", iperf_json_printf("id: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  retransmits: %d  max_snd_cwnd:  %d  max_rtt:  %d  min_rtt:  %d  mean_rtt:  %d", (int64_t) sp->id, (double) start_time, (double) end_time, (double) end_time, (int64_t) bytes_sent, bandwidth * 8, (int64_t) sp->result->stream_retrans, (int64_t) sp->result->stream_max_snd_cwnd, (int64_t) sp->result->stream_max_rtt, (int64_t) sp->result->stream_min_rtt, (int64_t) ((sp->result->stream_count_rtt == 0) ? 0 : sp->result->stream_sum_rtt / sp->result->stream_count_rtt)));
                iprintf(test, report_bw_retrans_format, sp->id, start_time, end_time, ubuf, nbuf, sp->result->stream_retrans, report_sender);
	    } else {
		/* Summary, TCP without retransmits. */
                cJSON_AddItemToObject(json_summary_subflow, "sender", iperf_json_printf("id: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f", (int64_t) sp->id, (double) start_time, (double) end_time, (double) end_time, (int64_t) bytes_sent, bandwidth * 8));
                iprintf(test, report_bw_format, sp->id, start_time, end_time, ubuf, nbuf, report_sender);
	    }
        }
/*
	if (sp->diskfile_fd >= 0) {
		...
		unit_snprintf(sbuf, UNIT_LEN, (double) sb.st_size, 'A');
		if (test->json_output)
		    cJSON_AddItemToObject(json_summary_subflow, "diskfile", iperf_json_printf("sent: %d  size: %d  percent: %d  filename: %s", (int64_t) bytes_sent, (int64_t) sb.st_size, (int64_t) percent, test->diskfile_name));
		else
		    iprintf(test, report_diskfile, ubuf, sbuf, percent, test->diskfile_name);
	    }
	}
*/
	...
	unit_snprintf(nbuf, UNIT_LEN, bandwidth, test->settings->unit_format);
	if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
            cJSON_AddItemToObject(json_summary_subflow, "receiver", iperf_json_printf("id: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f", (int64_t) sp->id, (double) start_time, (double) end_time, (double) end_time, (int64_t) bytes_received, bandwidth * 8));
            iprintf(test, report_bw_format, sp->id, start_time, end_time, ubuf, nbuf, report_receiver);
	}
    }
    }
...
print_sf_interval_results(struct iperf_test *test, struct iperf_subflow *sf, cJSON *json_interval_streams)
{
	...
	if (test->sender && test->sender_has_retransmits) {
        ...
        /* Interval, TCP without retransmits. */
            cJSON_AddItemToArray(json_interval_streams, iperf_json_printf(
                "id: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  omitted: %b",
                 (int64_t) sf->id, (double) st, (double) et, (double) irp->interval_duration, (int64_t) irp->bytes_transferred,
                 bandwidth * 8, irp->omitted));
            iprintf(test, report_bw_format, sf->id, st, et, ubuf, nbuf, irp->omitted?report_omitted:"");
	}
    }
}
